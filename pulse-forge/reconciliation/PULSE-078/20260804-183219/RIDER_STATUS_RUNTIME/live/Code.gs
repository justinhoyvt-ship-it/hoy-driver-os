...
/* ===== Code.gs ===== */
function pulseRequestPortalPage_(testMode) {
  const privateUrl=String(PropertiesService.getScriptProperties().getProperty('PULSE_FRIEND_REQUEST_URL')||'');
  if(!privateUrl){
    return HtmlService.createHtmlOutput('<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;min-height:100vh;display:grid;place-items:center;padding:24px;background:#07101c;color:#f7fbff;font-family:Arial,sans-serif}.card{max-width:520px;padding:28px;border:1px solid rgba(255,255,255,.16);background:#0d1a2d}h1{margin:0 0 10px;font-size:24px}p{margin:0;color:#a8b8cd;line-height:1.5}</style></head><body><main class="card"><h1>Ride-request link unavailable</h1><p>The private request app has not been connected to this portal yet.</p></main></body></html>')
      .setTitle('Ride request unavailable')
      .addMetaTag('viewport','width=device-width, initial-scale=1, viewport-fit=cover')
      .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  }
  const separator=privateUrl.indexOf('?')>=0?'&':'?';
  const framedUrl=privateUrl+(testMode?separator+'mode=test':'');
  const safeUrl=framedUrl.replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
  const html='<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#241335"><style>html,body,iframe{margin:0;width:100%;height:100%;border:0;background:#241335}body{overflow:hidden}iframe{display:block}</style></head><body><iframe title="Private ride request" src="'+safeUrl+'" allow="geolocation"></iframe></body></html>';
...
function doGet(e) {
  ...
  if(String(params.requestRide||'')==='1'){
    return pulseRequestPortalPage_(String(params.mode||'').toLowerCase()==='test');
  }
  return HtmlService.createHtmlOutput(PULSE_INDEX_HTML_V144)
    .setTitle('Pulse OS Driver')
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL)
    .addMetaTag('viewport','width=device-width, initial-scale=1, viewport-fit=cover');
}
...
const PULSE_FRIEND_RIDE_SHEET_='Ride Requests';
...
function friendRideRows_(){
  const sh=pulseSpreadsheet_().getSheetByName(PULSE_FRIEND_RIDE_SHEET_);
  if(!sh)return[];
  const values=sh.getDataRange().getValues();
  if(values.length<2)return[];
  const headers=values[0].map(String);
  const required=['Request ID','Received At','Updated At','Status','Customer Name','Customer Phone','Customer Email','Pickup Address','Destination','Pickup At','Passengers','Notes','Consent'];
  if(!required.every(function(header){return headers.indexOf(header)>=0;})){
    throw new Error('Ride Requests headers do not match the hardened request-app schema.');
  }
  return values.slice(1).filter(function(row){return row.some(function(v){return v!==''&&v!==null;});}).map(function(row){
    ...
    const contact=[String(item['Customer Phone']||''),String(item['Customer Email']||'')].filter(Boolean).join(' · ');
    return{
      requestId:String(item['Request ID']||''),
      source:String(item.Source||'PRIVATE_LINK'),
      createdAt:friendRideIso_(item['Received At']),
      updatedAt:friendRideIso_(item['Updated At']),
      requesterName:String(item['Customer Name']||''),
      contactMethod:contact,
      pickupAt:friendRideIso_(item['Pickup At']),
      pickupLocation:String(item['Pickup Address']||''),
      destination:String(item.Destination||''),
      passengerCount:Math.max(1,Number(item.Passengers)||1),
      notes:String(item.Notes||''),
      consent:normalizeBoolean_(item.Consent),
      status:String(item.Status||'REQUESTED').toUpperCase(),
      statusNote:String(item['Driver Notes']||'')
    };
  });
}
function friendRideDecisionConfig_(){
  const props=PropertiesService.getScriptProperties();
  const rawUrl=String(props.getProperty('PULSE_FRIEND_REQUEST_URL')||'');
  const baseUrl=rawUrl?rawUrl.split('?')[0]:'';
  const secret=String(props.getProperty('PULSE_FRIEND_CONFIRM_SECRET')||'');
  return{baseUrl:baseUrl,secret:secret,configured:!!baseUrl&&!!secret};
}
function friendRideDecisionToken_(action,requestId,exp,secret){
...
function friendRideDecisionUrl_(action,requestId,config){
  if(!config||!config.configured)return'';
  const exp=Math.floor(Date.now()/1000)+30*86400;
  const token=friendRideDecisionToken_(action,requestId,exp,config.secret);
  return config.baseUrl+'?action='+encodeURIComponent(action)+'&id='+encodeURIComponent(requestId)+'&exp='+exp+'&t='+encodeURIComponent(token);
}
function pulseGetFriendRides(){
  const decisionConfig=friendRideDecisionConfig_();
  ...
  rides.forEach(function(ride){
    ...
    ride.actions={
      configured:requested&&decisionConfig.configured,
      confirmUrl:requested?friendRideDecisionUrl_('confirm',ride.requestId,decisionConfig):'',
      declineUrl:requested?friendRideDecisionUrl_('decline',ride.requestId,decisionConfig):''
    };
  });
  const nextConfirmed=rides.find(function(ride){return ride.status==='CONFIRMED'&&new Date(ride.pickupAt)>=new Date();})||null;
  const props=PropertiesService.getScriptProperties();
  const privateRequestConfigured=!!String(props.getProperty('PULSE_FRIEND_REQUEST_URL')||'');
  const portalUrl=String(props.getProperty('PULSE_REQUEST_PORTAL_URL')||'');
  const requestConfigured=privateRequestConfigured&&!!portalUrl;
  return{
    checkedAt:new Date().toISOString(),
    requestUrl:requestConfigured?portalUrl:'',
    requestConfigured:requestConfigured,
    privateRequestConfigured:privateRequestConfigured,
    decisionConfigured:decisionConfig.configured,
    rides:rides,
    nextConfirmed:nextConfirmed,
    sourceSheet:PULSE_FRIEND_RIDE_SHEET_,
    writesPerformed:false
  };
}

function scheduledRideTripMatch_(trip,requestId){
  if(!trip)return false;
  const id=String(requestId||'');
  return String(trip.requestId||'')===id||String(trip.tripId||'')===id;
}
function pulseStartScheduledRide(payload){
  payload=payload||{};
  const requestId=String(payload.requestId||'').trim();
  if(!requestId)throw new Error('A confirmed request ID is required.');
  const ride=friendRideRows_().find(function(item){return item.requestId===requestId;});
  if(!ride)throw new Error('Scheduled ride request not found.');
  if(ride.status!=='CONFIRMED')throw new Error('Only a confirmed request can be started.');
  const before=tripStateSnapshot_();
  if(scheduledRideTripMatch_(before.active,requestId))return{ok:true,duplicate:true,reason:'already_active',requestId:requestId,pickupLocation:ride.pickupLocation,destination:ride.destination,scheduledPickupAt:ride.pickupAt,state:before};
  if(scheduledRideTripMatch_(before.queued,requestId))return{ok:true,duplicate:true,reason:'already_queued',requestId:requestId,pickupLocation:ride.pickupLocation,destination:ride.destination,scheduledPickupAt:ride.pickupAt,state:before};
  const result=transitionTrip({
    action:'ACCEPTED',
    clientEventId:'scheduled:'+requestId,
    tripId:requestId,
    requestId:requestId,
    reservation:true,
    platform:'Pulse Reservation',
    startedAt:new Date().toISOString(),
    scheduledPickupAt:ride.pickupAt||'',
    pickupLocation:ride.pickupLocation,
    destination:ride.destination,
    notes:safeJson_({requestId:requestId,requesterName:ride.requesterName,pickupAt:ride.pickupAt,pickupLocation:ride.pickupLocation,destination:ride.destination})
  });
  pulseClearMobileCache_();
  return Object.assign({requestId:requestId,pickupLocation:ride.pickupLocation,destination:ride.destination,scheduledPickupAt:ride.pickupAt,writeTarget:'trip-state-only'},result);
}

function pulseReleaseBaseline(){
  const props=PropertiesService.getScriptProperties();
  const requiredPropertyNames=['PULSE_FRIEND_REQUEST_URL','PULSE_REQUEST_PORTAL_URL','PULSE_FRIEND_CONFIRM_SECRET'];
  const propertyState={};
  requiredPropertyNames.forEach(function(name){propertyState[name]=!!String(props.getProperty(name)||'');});
  const requestSheet=pulseSpreadsheet_().getSheetByName(PULSE_FRIEND_RIDE_SHEET_);
  return{
    app:PULSE.APP_NAME,
    version:PULSE.VERSION,
    checkedAt:new Date().toISOString(),
    spreadsheetId:PULSE.SPREADSHEET_ID,
    sourceBaseline:'main@4bbd8c869dd9fba4af6776604cbf1dfbdf6c07d6',
    sourcePath:'pulse-autobuild/runtime/Code.gs',
    requiredProperties:propertyState,
    functions:{
      pulseRuntimeCapabilities:typeof pulseRuntimeCapabilities==='function',
      pulseGetFriendRides:typeof pulseGetFriendRides==='function',
      pulseStartScheduledRide:typeof pulseStartScheduledRide==='function',
      pulseFieldPreflight:typeof pulseFieldPreflight==='function'
    },
    rideRequestsSheetPresent:!!requestSheet,
    writesPerformed:false
  };
}

function pulseRuntimeCapabilities(){
  return{
    app:PULSE.APP_NAME,
    version:PULSE.VERSION,
    checkedAt:new Date().toISOString(),
    spreadsheetId:PULSE.SPREADSHEET_ID,
    functions:{
      pulseRuntimeCapabilities:true,
      pulseGetMobileState:typeof pulseGetMobileState==='function',
      pulseGetLiveSignals:typeof pulseGetLiveSignals==='function',
      pulseGetMobileHistory:typeof pulseGetMobileHistory==='function',
      pulseGetTripState:typeof pulseGetTripState==='function',
      pulseTripAction:typeof pulseTripAction==='function',
      pulseStartMobileShift:typeof pulseStartMobileShift==='function',
      pulseEndMobileShift:typeof pulseEndMobileShift==='function',
      pulseLogMobilePing:typeof pulseLogMobilePing==='function',
      pulseRefreshMobileData:typeof pulseRefreshMobileData==='function',
      pulseGetFriendRides:typeof pulseGetFriendRides==='function',
      pulseStartScheduledRide:typeof pulseStartScheduledRide==='function',
      pulseReleaseBaseline:typeof pulseReleaseBaseline==='function',
      pulseFieldPreflight:typeof pulseFieldPreflight==='function'
    },
    writesPerformed:false
  };
...
function pulseFieldPreflight() {
  ...
  const endpoints={
    ...
    friendRides:typeof pulseGetFriendRides==='function',
    scheduledRideStart:typeof pulseStartScheduledRide==='function',
    releaseBaseline:typeof pulseReleaseBaseline==='function'
  };
