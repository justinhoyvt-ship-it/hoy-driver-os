...
function transitionTrip(p){p=p||{};const action=String(p.action||'').toUpperCase();if(!PULSE_TRIP_STATES.includes(action))throw new Error('Invalid trip action');const lock=LockService.getUserLock();lock.waitLock(10000);try{let active=activeTripState_(),queued=queuedTripState_();if(action==='OFFER_RECEIVED')return{ok:true,state:tripStateSnapshot_()};if(action==='ACCEPTED'){if(active){if(queued)return{ok:true,duplicate:true,reason:'queue_occupied',state:tripStateSnapshot_()};queued=newTripState_(p,'QUEUED_ACCEPTED');writeTripState_('pulseQueuedTrip',queued);return{ok:true,queued:true,state:tripStateSnapshot_()};}active=newTripState_(p,'ACCEPTED');writeTripState_('pulseActiveTrip',active);return{ok:true,queued:false,state:tripStateSnapshot_()};}if(action==='CANCELLED'){if(active){auditTrip_(active,'CANCELLED',p);writeTripState_('pulseActiveTrip',null);promoteQueuedTrip_();}else if(queued){auditTrip_(queued,'CANCELLED',p);writeTripState_('pulseQueuedTrip',null);}return{ok:true,state:tripStateSnapshot_()};}if(!active){if(normalizeBoolean_(p.reservation)){active=newTripState_(p,'ACCEPTED');writeTripState_('pulseActiveTrip',active);}else return{ok:false,needsReconciliation:true,reason:'no_active_trip',state:tripStateSnapshot_()};}auditTrip_(active,action,p);if(action==='PICKED_UP'||action==='ON_TRIP'){if(p.pickupZoneId)active.pickupZoneId=String(p.pickupZoneId);if(p.effectiveAt)active.startedAt=String(p.effectiveAt);writeTripState_('pulseActiveTrip',active);return{ok:true,state:tripStateSnapshot_()};}if(action==='DROPPED_OFF'||action==='COMPLETED'){const result=completeTripState_(active,p);if(!result.ok){writeTripState_('pulseActiveTrip',result.state);return result;}writeTripState_('pulseActiveTrip',null);const promoted=promoteQueuedTrip_();return{ok:true,completed:true,tripId:result.tripId,promoted:!!promoted,state:tripStateSnapshot_()};}writeTripState_('pulseActiveTrip',active);return{ok:true,state:tripStateSnapshot_()};}finally{lock.releaseLock();}}
